...
All 15 binding staging MVP criteria now have durable evidence. The simplified curated-admission boundary, generated identities, accepted-Publisher onboarding, Integration Declaration, runtime-bound SDK connection, hosted Stripe test Checkout, signed/replay-safe billing projection, cancellation, failed renewal, paid-through expiry, real extension activation, entitlement failures, Cash Receipt, balanced Allocation, accrued Publisher Earning, secret-safe tracing, and D1 recovery all pass at their required boundaries. The correctly attributed accrued Earning is the staging money boundary; actual Publisher payment remains a controlled live-pilot operation. Stripe account `acct_1MwbFJI9EPtyKcIs` (**SERP Pass**) is the only configured sandbox. Production remains absent and the live gate is closed.
...
| Local workerd | passed | OpenNext `1.19.9` Worker reads migrated local D1; rendered Chromium passes human auth, role denials, Operator bootstrap, Application/Product Acceptance, Integration Declaration, runtime-bound connection verification, normalized billing, real extension activation/entitlement, immutable allocation posting, Operator-only external Publisher Payment recording, replay/conflict/immutability checks, and D1 rate limiting at `localhost:8788`. Dormant package-review and Connect/Transfer experiments remain historical evidence. All 27 migrations apply to the persistent local database. |
| Cloudflare staging | deployed and passed | Current deployment `37759952-c61e-4f92-bdd2-24599944a0be`, isolated APAC D1, health `200 ready`, and all 27 migrations current. Public Home, Apps, Submit, and Docs surfaces expose the four-step Application → Product Acceptance → integration registration → connection flow. John Doe Focus Timer (`bpjn…kobdb`) and Invited Publisher Video Notes (`pken…kahf`) both connect from real unpacked Chromium origins and resolve through canonical authority. John's linked Subscriber has a signed-event-derived test Subscription, one paid Invoice/Cash Receipt, `active` entitlement, and a balanced `$7/$2/$1` Allocation. Real hosted Checkout/Portal/Event projection, test-clock renewal failure/expiry, Operator-only Payment route, and simplified Publisher UI also pass. Connect onboarding and Stripe Transfers are disabled. Both staged `$7` Earnings correctly remain accrued with zero Payment rows. |
...
- An authorized direct Stripe API acceptance run verified exact Account `acct_1MwbFJI9EPtyKcIs` and test Price `price_1U3zRcI9EPtyKcIsot42EEhL`, used Stripe's `pm_card_visa` test PaymentMethod, and created Subscription `sub_1U4O88I9EPtyKcIs7zzm0R2T`. The API response alone did not grant access: staging recorded applied `invoice.paid` and `subscription.updated` webhook Events, one Invoice, one Cash Receipt, and paid-through `2026-09-14T16:38:52Z` before the existing John Doe App session returned `active`. The test Subscription is scheduled to cancel at period end; a third signed Event projected `cancel_at_period_end` while entitlement correctly remained `active` through the paid-through date.
- A separate Stripe test clock created a paid period ending 2026-07-01, then advanced through renewal with the documented decline-after-attach PaymentMethod. Real signed provider Events produced one failed Invoice, left exactly one Cash Receipt, preserved `entitled_until=1782864000`, and returned `inactive`. Finishing the disposable simulation projected `canceled` without erasing the failed-renewal/expiry evidence.
...
- The accepted Publisher registers only the versioned public manifest, built/published version, and real Distribution identity. No source or ZIP is required by the active MVP route.
- Registration remains disconnected and catalog-ineligible until the extension calls the SDK connection endpoint from the exact declared `chrome-extension://<runtime-id>` origin.
- A successful connection records first/latest timestamps and a count, promotes the accepted App/Distribution into canonical authority, and is idempotent on repeat calls.
- The Operator and Publisher surfaces distinguish waiting from connected without offering a second approval action.
- Connection verification proves protocol identity/connectivity only; it does not certify package safety or a Publisher's local feature enforcement.
...
- Chromium loads the current source with stable runtime ID `pkenmpgagmnabdljhnjibgpbmakekahf`; staging recognizes it under Apps Pass-generated identities `app_invited_publisher_video_notes` and `pub_invited_publisher_pilot` only after the real extension records runtime-bound connection evidence.
- The same rendered-browser flow passes locally and against deployed staging; the fixed real-extension staging check is safely repeatable once connected.
...
- The independently built Publisher extension creates its proof challenge from its actual `chrome-extension://pkenmpgagmnabdljhnjibgpbmakekahf` origin, receives an expiring activation URL, and stores its installation and opaque App-session token in `chrome.storage.local`.
...
- John Doe Focus Timer has its own staging App Link and hash-only App session for the signed-in staging Subscriber. Before billing, the real extension returned truthful `inactive`. After the authorized Stripe API test produced signed provider Events and a normalized paid-through Subscription for that same Subscriber, the unchanged linked session returned `active` and unlocked the demonstration timer. D1 stores a 64-character token hash and no raw App-session token. The resumable `walkthrough:john-doe:activation` helper preserves the project browser across prepare, human approval, finish, and repeatable checks.
...
- Unpaid staging Subscribers still receive `inactive`; the exact paid John Doe Subscriber now receives `active` directly from the shared authority without relinking.
...
- The John Doe API purchase created Cash Receipt `receipt:test:in_1U4O88I9EPtyKcIsF1n0X3N9`. Protected Operator Allocation Run `alloc_john_api_1U4O88_20260815` posted `$7.00` to John Doe Studio as `earning_john_api_1U4O88_20260815`, `$2.00` to the platform, and `$1.00` to reserve. The immutable ledger sums to zero and the Earning has zero Publisher Payment rows.
...
- A real invited Publisher independently reviewing the SDK handoff. The first actual external Publisher payment and its recorded evidence are deferred to the controlled live-Publisher settlement gate rather than fabricated in staging.
