...
Slices 1–3 are complete with the simplified curated-admission boundary: public Application, reasoned Operator Product Acceptance, generated identities, accepted-Publisher onboarding, authenticated Integration Declaration, and runtime-bound SDK connection now pass locally and on deployed staging. No ZIP/source upload or second technical approval is required or claimed. Slice 4 has real Stripe test-mode Checkout, an additional exact-account API Subscription, signed Event projection, replay evidence, one Cash Receipt per paid Invoice, and rendered Portal cancellation. Slice 5 now proves the already-linked John Doe extension changes from `inactive` to `active` only after its Subscriber receives normalized paid-through authority. Slice 6 keeps Publisher payment instructions outside Apps Pass. Slice 7 has two real-receipt `$7/$2/$1` test Allocations, including a `$7` John Doe Studio Earning; this correctly attributed accrued Earning is the staging MVP boundary. The locally verified Publisher Payment record remains supporting code for the first controlled real-Publisher settlement, not a staging acceptance gate. Stripe account `acct_1MwbFJI9EPtyKcIs` (**SERP Pass**) is the only configured sandbox. Production remains absent.
