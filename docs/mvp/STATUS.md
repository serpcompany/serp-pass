...
| Cloudflare staging | deployed and passed | Worker version `766fb1b0-02a2-4e45-86b8-b7b65b2ffa52`, isolated APAC D1, health `200 ready`, and all 27 migrations current. Public Home, Apps, Submit, and Docs surfaces expose the four-step Application → Product Acceptance → integration registration → connection flow. John Doe Focus Timer connected from real unpacked Chromium runtime `bpjnchabpcjomgncmbgphbdggkgkobdb`; D1 recorded first/latest connection evidence and the canonical authority endpoint returns the accepted App/Distribution. Its linked Subscriber now has a signed-event-derived test Subscription, one paid Invoice/Cash Receipt, `active` entitlement, and a balanced `$7/$2/$1` Allocation. Real hosted Checkout/Portal/Event projection, Operator-only Payment route, and simplified Publisher UI also pass. Connect onboarding and Stripe Transfers are disabled. Both staged `$7` Earnings correctly remain accrued with zero Payment rows. |
