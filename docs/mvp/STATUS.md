...
| Cloudflare staging | deployed and passed | [`serp-apps-pass-staging.serpcompany.workers.dev`](https://serp-apps-pass-staging.serpcompany.workers.dev), Worker version `a07e6812-ceb8-45eb-91b3-954cbc77befa`, isolated `apps-pass-staging` D1 in APAC, health `200 ready`, all 24 migrations current, real Stripe Checkout/Portal/Event projection, protected Operator trace, real paid extension activation, and real-receipt Allocation pass. Connect onboarding is enabled only here; test Transfers remain disabled. |
...
| Stripe | configured test sandbox | Exact account `acct_1MwbFJI9EPtyKcIs` (**SERP Pass**) was verified before mutation. One Product, `$10/month` Price, Portal configuration, platform webhook, and Connect webhook exist in test mode. Real test Customers/Subscriptions/Invoices/Events and one `$10` Apps Pass Allocation now exist. Stripe rejected connected-Account creation until the platform completes Connect signup; no connected Account, Transfer, live object, real card, or bank account exists. Full inventory: [STRIPE_SANDBOX_STATE.md](./STRIPE_SANDBOX_STATE.md). |
...
- `pnpm mvp:connect:test` passes against local workerd without any Stripe API request. Migration `0024` and the onboarding route are deployed; the dedicated Connect webhook is configured and rejects unsigned requests. Onboarding is enabled only in staging for the approved `US` test; there are zero connected Accounts, Connect Events, Payouts, and Payout Events until the platform completes Connect signup and hosted onboarding succeeds.
...
- A settlement-ready connected Account, real Stripe test Transfer/full reversal, or bank Payout. The real-receipt Allocation and Publisher Earning now exist in staging; Stripe Connect platform signup is the remaining prerequisite for hosted Publisher onboarding.
