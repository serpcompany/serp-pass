...
Wrangler `4.122.0` returned `incomplete input` while applying trigger-rich migrations through the normal remote migration query path. This affected `0017_earnings_allocation_ledger.sql`, migrations `0019` through `0023`, and `0025_external_publisher_payment.sql`. Each unchanged file first passed against both persistent local D1 and a fresh empty local D1. Staging was checked after every failed normal-path attempt and showed no partial schema or migration record.
