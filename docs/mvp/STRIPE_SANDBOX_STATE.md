...
The same authorization was used for one bounded Stripe test-clock lifecycle simulation. Test clock `clock_1U4OxKI9EPtyKcIsuVRr7II6` began at 2026-06-01, created test Customer `cus_V4Y3GxWLlAwoly`, Subscription `sub_1U4OxMI9EPtyKcIsMfoClyjG`, and paid Invoice `in_1U4OxMI9EPtyKcIshs8gAJIr`, then changed the Customer to Stripe's documented decline-after-attach PaymentMethod. Advancing through renewal produced failed Invoice `in_1U4OxVI9EPtyKcIssywkGUCP`. Signed provider Events made staging `past_due` without extending `entitled_until=1782864000` or creating another Cash Receipt; because that paid-through time was already past, access was `inactive`. Finishing the simulation deleted its disposable Stripe objects and projected `canceled` while D1 retained one failed Invoice, one Cash Receipt, the unchanged paid-through time, and the inactive decision as durable evidence.
