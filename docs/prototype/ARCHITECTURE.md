...
The proof tests answer narrow architectural questions (paths below are relative to the repository root):
...
They do not claim production coverage, browser-store compatibility, billing correctness, authentication security, uptime, scalability, or operational readiness. See `tests/README.md`.

## Evidence versus current files

The decisive proof is preserved by Git commits and the recorded migration checksum in [FREEZE.md](./FREEZE.md). Later documentation and naming cleanup does not rewrite that evidence. The observed results are recorded in [EVALUATION.md](./EVALUATION.md).
